...
public class OperationProviderLibraryImpl implements OperationProviderLibrary {
	/** The {@link Logger}. */
	private final Logger logger = LoggerFactory.getLogger(OperationProviderLibraryImpl.class);
	...
	public CreateBookResponseType createBook(
		...
		try {
			...
			this.logger.debug(" createBook duration {}", requestDuration(start));
			this.logger.debug("-createBook");
		}
		return value;
	}
	 ...
	 */
	@Override
	public GetBookResponseType getBook(
			final GetBookRequestType request) {
		...
		try {
			...
			JAXBElement<GetBookResponseType> response = this.getBook
					.invoke(this.of
							.createGetBookRequest(request));
			if (response != null) {
				value = response.getValue();
			}
		} finally {
			this.requestFinalization();
			this.logger.debug(" getBook duration {}", requestDuration(start));
			this.logger.debug("-getBook");
		}
		return value;
	}
