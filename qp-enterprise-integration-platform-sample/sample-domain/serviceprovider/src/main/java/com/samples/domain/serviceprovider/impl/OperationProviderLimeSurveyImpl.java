 ...
 */
@Component
public class OperationProviderLimeSurveyImpl implements OperationProviderLimeSurvey {
	/** The {@link Logger}. */
	private final Logger logger = LoggerFactory.getLogger(OperationProviderLimeSurveyImpl.class);
	...
	public GetSessionKeyResponseType getSessionKey(
		...
		try {
			...
			JAXBElement<GetSessionKeyResponseType> response = this.getSessionKey
					.invoke(this.of
							.createGetSessionKeyRequest(request));
			if (response != null) {
				value = response.getValue();
			}
		} finally {
			this.requestFinalization();
			this.logger.debug(" getSessionKey duration {}", requestDuration(start));
			this.logger.debug("-getSessionKey");
		}
		return value;
	}
	...
	public ListSurveysResponseType listSurveys(
		...
		try {
			...
			JAXBElement<ListSurveysResponseType> response = this.listSurveys
					.invoke(this.of
							.createListSurveysRequest(request));
			if (response != null) {
				value = response.getValue();
			}
		} finally {
			this.requestFinalization();
			this.logger.debug(" listSurveys duration {}", requestDuration(start));
			this.logger.debug("-listSurveys");
		}
		return value;
	}
