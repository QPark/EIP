...
public class ElementType {
	...
	this.classNameFqObject = new StringBuffer(128).append(this.packageName).append(".")
		.append(this.getClassNameObject()).toString();

	/* Service Operation definition. */
	int index = this.getClassNameObject().lastIndexOf(config.getServiceRequestSuffix());
	if (index > 0) {
	    this.isRequest = true;
	    this.operationName = this.getClassNameObject().substring(0, index);
	} else {
	    this.isRequest = false;
	    index = this.getClassNameObject().lastIndexOf(config.getServiceResponseSuffix());
	    if (index > 0) {
		this.operationName = this.getClassNameObject().substring(0, index);
	    } else {
		this.operationName = "";
	...
	if (this.operationName.length() > 0) {
	    this.methodName = new StringBuffer().append(this.operationName.substring(0, 1).toLowerCase())
		    .append(this.operationName.substring(1, this.operationName.length())).toString();
	} else {
	    this.methodName = "";
	}
	...
	if (this.operationName.length() > 0) {
	    this.classNameMockOperationProvider = new StringBuffer(128).append(this.operationName)
		    .append("MockOperation").toString();
	} else {
	    this.classNameMockOperationProvider = new StringBuffer(128).append(this.element.getName().getLocalPart())
		    .append("MockOperation").toString();
	}
	if (this.packageName.contains(config.getMessagePackageNameSuffix())) {
	    this.packageNameMockOperationProvider = this.packageName.substring(0,
		    this.packageName.length() - config.getMessagePackageNameSuffix().length() - 1);
	} else {
	...
	if (this.operationName.length() > 0) {
	    this.classNameGateway = new StringBuffer(128).append(this.operationName).toString();
	} else {
	    this.classNameGateway = new StringBuffer(128).append(this.element.getName().getLocalPart()).toString();
	}
	if (this.packageName.contains(config.getMessagePackageNameSuffix())) {
	    ...
	    this.packageNameGateway = new StringBuffer(128).append(this.packageName).append(".gateway").toString();
	}
	if (this.element.getAnnotation() != null && this.element.getAnnotation().getUserInformation() != null
		&& this.element.getAnnotation().getUserInformation().length > 0) {
	    StringBuffer sb = new StringBuffer(124);
	    for (XmlObject u : this.element.getAnnotation().getUserInformation()) {
	...
	this.classNameFqGateway = new StringBuffer(128).append(this.packageNameGateway).append(".")
		.append(this.getClassNameGateway()).toString();

	/* Service id definition. */
	this.serviceId = ServiceIdRegistry.getServiceId(this.packageName, this.getTargetNamespace(),
		config.getMessagePackageNameSuffix(), config.getDeltaPackageNameSuffix());

	this.beanIdOperationProvider = new StringBuffer(64).append("operationProvider")
		.append(ServiceIdRegistry.capitalize(this.serviceId)).append(this.operationName).toString();
	this.beanIdMockOperationProvider = new StringBuffer(this.beanIdOperationProvider.length() + 4)
		.append(this.beanIdOperationProvider).append("Mock").toString();

	String serviceChannelNameStart = new StringBuffer(16).append(WEB_SERVICE_CHANNEL_NAME_PREFIX)
		.append(ServiceIdRegistry.capitalize(this.serviceId)).toString();
	String operationChannelNameStart = new StringBuffer(64).append(serviceChannelNameStart)
		.append(this.operationName).append(WEB_SERVICE_CHANNEL_NAME_CONTENT).toString();

	this.channelSecurityPatternService = new StringBuffer(64).append(serviceChannelNameStart).append(".*?")
		.append(WEB_SERVICE_CHANNEL_NAME_CONTENT).append(".*").toString();
	this.channelSecurityPatternOperation = new StringBuffer(64).append(operationChannelNameStart).append(".*")
		.toString();

	this.channelNameWsRequest = new StringBuffer(64).append(operationChannelNameStart)
		.append(WEB_SERVICE_CHANNEL_NAME_REQUEST_SUFFIX).toString();
	this.channelNameWsResponse = new StringBuffer(64).append(operationChannelNameStart)
		.append(WEB_SERVICE_CHANNEL_NAME_RESPONSE_SUFFIX).toString();

	this.beanIdWsInboundGateway = new StringBuffer(64).append(this.serviceId).append(this.operationName)
