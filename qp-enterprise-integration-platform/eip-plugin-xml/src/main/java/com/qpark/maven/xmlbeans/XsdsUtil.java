...
public class XsdsUtil {
	...
	public static ComplexType findRequest(final ComplexType response,
		...
		int index = response.getClassName().lastIndexOf(
	...
	private static void setupXsdContainerTotalImports(
		...
		for (String importedTargetNamespace : container
			...
			if (!container.getTotalImportedTargetNamespaces().contains(
					importedTargetNamespace)) {
				child = map.get(importedTargetNamespace);
				if (child != null) {
					container.getTotalImportedTargetNamespaces().add(
	...
	private static void testGetServiceIdTree(final XsdsUtil xsds) {
		...
		System.out.println(ServiceIdRegistry.isValidServiceId("directory",
				"directory.v20"));
		System.out.println(ServiceIdRegistry.isValidServiceId("common",
				"directory.v20"));
	...
	private static void testPrintComplexTypeContent(final XsdsUtil xsds,
		...
		for (ComplexType ct : xsds.getComplexTypes()) {
			...
			if (ct.getType().getName() != null
					&& !notImportedModels.containsKey(ct.getType().getName()
							.getNamespaceURI())) {
	...
	private static void testPrintComplexTypeResponseSample(final XsdsUtil xsds,
		...
		for (ComplexType ct : xsds.getComplexTypes()) {
			if (ct.getClassName().equals(op + "ResponseType")) {
				...
				try {
					JAXBContext jaxbContext = JAXBContext.newInstance(ct
							.getPackageName());
	...
	private static void testPrintElementTypeRequestResponse(
			final XsdsUtil xsds, final Map<String, String> notImportedModels,
		...
		for (ElementType et : xsds.getElementTypes()) {
			...
			if (et.isRequest()) {
				...
				if (et.getTargetNamespace() != null
						&& !notImportedModels.containsKey(et
								.getTargetNamespace())) {
	...
	private static void testPrintComplexTypeRequestResponse(
			final XsdsUtil xsds, final Map<String, String> notImportedModels,
			final boolean onlyUsed) {
		StringBuffer sb = new StringBuffer(128);
		...
		for (ComplexType ct : xsds.getComplexTypes()) {
			...
			if (ct.isRequestType()) {
				...
				if (ct.getTargetNamespace() != null
						&& !notImportedModels.containsKey(ct
								.getTargetNamespace())) {
					sb.insert(0, "ComplexType imported     ");
					used = true;
				} else {
	...
	private static void testXsdContainers(final File dif) {
		...
		for (Entry<String, XsdContainer> entry : containers.entrySet()) {
			...
			System.out.println("\t" + entry.getValue().getPackageName());
	...
	public static void printCatalog(final XsdsUtil xsds, final String pathPrefix) {
		for (ComplexType ct : xsds.getComplexTypes()) {
			if (ct.getClassName().equals(
					"MonicsSatelliteInclinedOrbitFlagMappingType")) {
				...
				if (app != null && app.length > 0) {
					...
					System.out.println("  getUserInformation: "
							+ app[0].getDomNode() + "=\"" + app[0].getClass()
							+ "\"");
				}
			}
			if (xsds.getXsdContainerMap(ct.getTargetNamespace()) == null) {
				...
				System.out.println(ct.getClassNameFullQualified()
						+ " "
						+ xsds.getXsdContainerMap(ct.getTargetNamespace())
								.getFile().getAbsolutePath()
								.replace(pathPrefix, ""));
			}
		}
		...
		for (ElementType et : xsds.getElementTypes()) {
			if (et.isRequest()) {
				System.out.println(et.getServiceId()
						+ " "
						+ ServiceIdRegistry.isValidServiceId(et.getServiceId(),
								serviceId));
			}
		}
	}
	...
	public static String getXmlObjectAsSetterParam(final String objAndSetter,
		...
		if (XmlString.class.isInstance(obj) || XmlAnyURI.class.isInstance(obj)) {
			...
			sb.append(objAndSetter)
					...
					.append(obj.getStringValue()).append("\"));");
			sb.append(" } catch (javax.xml.datatype.DatatypeConfigurationException datatypeConfigurationException) { datatypeConfigurationException.printStackTrace(); }");
		} else {
			sb.append(objAndSetter).append("(").append(obj.getStringValue())
					.append(");");
		}
	...
	public static Class<?> getBuildInJavaClass(final QName qName) {
		Class<?> javaType = null;
		if (qName != null
				&& QNAME_BASE_SCHEMA_NAMESPACE_URI.equals(qName
						.getNamespaceURI())) {
