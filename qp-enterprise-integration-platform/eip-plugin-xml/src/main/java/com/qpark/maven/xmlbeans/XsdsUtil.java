...
public class XsdsUtil {
    ...
    private static String findNamespace(final String xml, final String targetNamespace) {
	String prefix = "";
    ...
    public static String getSampleCodeing(final SchemaType type, final String elementName) {
	...
	if (type.getName() != null && type.getName().getNamespaceURI() != null) {
		...
		xml = xml.replace("<xml-fragment", new StringBuffer(32).append("<").append(elementName)
			.append(" xmlns=\"").append(type.getName().getNamespaceURI()).append("\"").toString());
    ...
    public static SchemaTypeSystem getSchemaTypeSystem(final File f, final EntityResolver entityResolver) {
	...
	if (targetNameSpace != null && packageName != null) {
	    xsdContainer = new XsdContainer(f, baseDirectory, packageName, targetNameSpace, imports);
	}
	return xsdContainer;
    }
	...
	for (Entry<String, XsdContainer> xx : xsds.entrySet()) {
	    if (!isMessagePackageName(xx.getValue().getPackageName(), messagePackageNameSuffix)) {
		notImportedModels.put(xx.getKey(), xx.getValue().getFile().getAbsolutePath());
	    }
	}
	for (Entry<String, XsdContainer> xx : xsds.entrySet()) {
	...
	System.out.println(
		sb.toString() + entry.getServiceId() + " " + entry.getPackageName() + " " + entry.getTargetNamespace());
	...
	for (XsdContainer xx : xsds.getXsdContainerMap().values()) {
		...
		XsdContainer imported = xsds.getXsdContainerMap(importedTargetNamespace);
		if (imported == null) {
	...
	for (ComplexType ct : xsds.getComplexTypes()) {
		...
		System.out
			.println(ct.getClassNameFullQualified() + " " + xsds.getXsdContainerMap(ct.getTargetNamespace())
	...
	XsdsUtil xsds = new XsdsUtil(dif, "a.b.c.bus", messagePackageNameSuffix, "delta");
	System.out.println(Util.getDuration(System.currentTimeMillis() - start) + " needed for new XsdsUtil(...)");
	XsdsUtil config = xsds;
	Map<String, String> notImportedModels = getNotImportedModels(xsds, messagePackageNameSuffix);
	...
	for (ComplexType ct : xsds.getComplexTypes()) {
		...
		if (t != null && t.getName() != null) {
		    ...
		    System.out.println(sp.getMinOccurs() + " " + t.getName().getLocalPart() + " "
			    + getBuildInBaseType(t) + "  " + getBuildInBaseTypeClass(t).getSimpleName());
		} else {
    ...
    private final TreeSet<ComplexType> complexTypes = new TreeSet<ComplexType>(new Comparator<ComplexType>() {
	@Override
    ...
    private final TreeSet<ElementType> elementTypes = new TreeSet<ElementType>(new Comparator<ElementType>() {
	@Override
    ...
    public XsdsUtil(final File baseDirectory, final String basePackageName, final String messagePackageNameSuffix,
    ...
    /** The {@link Logger}. */
    private final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(XsdsUtil.class);
	...
	this.logger.debug("{} Got xsd files", Util.getDuration(System.currentTimeMillis() - start));
	this.xsdContainerMap = setupXsdContainers(this.xsdFiles, baseDirectory);
	this.logger.debug("{} Got xsd containers", Util.getDuration(System.currentTimeMillis() - start));
	int i = 0;
	...
	this.logger.info("Setup serviceIds: {}", ServiceIdRegistry.getAllServiceIds());
	...
	this.logger.debug("{} Init complex type children", Util.getDuration(System.currentTimeMillis() - start));
    }
    ...
    private void setupComplexTypes(final ComplexType ct, final TreeSet<ComplexType> complexTypes) {
