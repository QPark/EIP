...
public class SpringSelfcallingIntegrationConfigMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		File f = Util.getFile(this.outputDirectory,
				"SelfCallingIntegrationConfig.xml");
		this.getLog()
	...
	private String getSelfCallingConfig(final XsdsUtil config) {
				...
				if (elementResponse != null) {
					...
					if (ctResponse != null && !ctResponse.isSimpleType()
						...
						gateways.append(element
								.getClassNameFullQualifiedGateway());
						gateways.append("\"\n");
						...
						channels.add(new StringBuffer(256)
								.append("<int:channel id=\"internalSelfCallingWs")
								.append(capitalizedServiceId)
								.append("RequestChannel\" />").toString());
						channels.add(new StringBuffer(256)
								.append("<int:channel id=\"internalSelfCallingWs")
								.append(capitalizedServiceId)
								.append(element.getOperationName())
								.append("ResponseChannel\" />").toString());
						List<String> mappings = routerMappings.get(element
								.getServiceId());
						if (mappings == null) {
							mappings = new ArrayList<String>();
							routerMappings
									.put(element.getServiceId(), mappings);
						}
						mappings.add(new StringBuffer(256).append("")
								.append("<int:mapping value=\"")
								.append(ctResponse.getClassNameFullQualified())
								.append("\" channel=\"internalSelfCallingWs")
								.append(capitalizedServiceId)
								.append(element.getOperationName())
								.append("ResponseChannel\" />").toString());
					}
		...
		for (String serviceId : routerMappings.keySet()) {
			...
			sb.append(capitalizedServiceId);
			sb.append("JAXBElementAwarePayloadTypeRouter\" class=\"com.qpark.eip.core.spring.JAXBElementAwarePayloadTypeRouter\"/>\n");
		...
		sb.append("\t<bean id=\"internalSelfCallingWsPayloadLogger\" class=\"com.qpark.eip.core.spring.PayloadLogger\" >\n");
		sb.append("\t\t<property name=\"loggerName\" value=\"");
		sb.append(config.getBasePackageName());
		sb.append(".internal.self.calling.webservice\"/>\n");
		sb.append("\t</bean>\n");

		sb.append("\t<bean id=\"internalSelfCallingWsWss4jSecurityInterceptor\" class=\"org.springframework.ws.soap.security.wss4j.Wss4jSecurityInterceptor\">\n");
		...
		sb.append("\t\t<property name=\"securementPassword\" value=\"${eip.internal.self.calling.password:password}\" />\n");
		...
		sb.append("\t<bean id=\"internalSelfCallingWsSoap12MessageFactory\" class=\"org.springframework.ws.soap.saaj.SaajSoapMessageFactory\">\n");
