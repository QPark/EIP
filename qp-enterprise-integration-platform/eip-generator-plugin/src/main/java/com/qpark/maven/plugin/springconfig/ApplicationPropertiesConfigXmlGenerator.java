...
public class ApplicationPropertiesConfigXmlGenerator {
	...
	public ApplicationPropertiesConfigXmlGenerator(final String basePackageName,
		...
		this.placeholderConfigurerImpl = placeholderConfigurerImpl == null
				|| placeholderConfigurerImpl.trim().length() == 0
						? "com.qpark.eip.core.spring.ApplicationPlaceholderConfigurer"
	...
	public void generate() {
		...
		File f = Util.getFile(this.outputDirectory,
				new StringBuffer(32).append(this.basePackageName)
						.append(".properties-config.xml").toString());
		...
		try {
			...
			sb.append("# Properties of the ").append(prefix)
					.append("DataSourceJdbc default object:\n");
	...
	private Properties getGeneratedProperties() {
		...
		appl.setProperty("eip.application.name", p.getProperty(
				"service.application.name", "applicationNameNotApplicable"));
		appl.setProperty("eip.service.name",
				p.getProperty("service.name", "common"));
		appl.setProperty("eip.service.version",
				p.getProperty("service.version", "trunk"));
		appl.setProperty("eip.application.war.name",
				p.getProperty("service.war.name"));

		appl.setProperty("eip.application.build.time",
				Util.getXsdDateTime(new Date()));
		appl.setProperty("eip.application.scm.revision", this.revisionNumber);

		appl.setProperty("eip.application.maven.artifact.groupid",
				this.project.getGroupId());
		appl.setProperty("eip.application.maven.artifact.artifactid",
	...
	private String getPropertiesDefinition() {
		...
		sb.append(
				"\tFirst the versions and the defaults will be taken out of the classpath.\n");
		sb.append(
				"\t	If possible enhance and overwrite with files out of the\n");
		sb.append("\t${catalina.base}/conf/");
		sb.append(this.basePackageName);
		sb.append("*.property files.\n");
		sb.append(
				"\tAt last the username and password properties will be taken from\n");
		sb.append("\t${catalina.base}/conf/");
		sb.append(this.basePackageName);
		...
		sb.append(
				"\t\t\t\t<value>classpath*:eip.generated.properties</value>\n");
		sb.append("\t\t\t\t<value>classpath*:");
		sb.append(this.basePackageName);
