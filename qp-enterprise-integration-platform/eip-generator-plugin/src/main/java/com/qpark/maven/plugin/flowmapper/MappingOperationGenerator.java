 ...
 * @author bhausen
 */
public class MappingOperationGenerator extends AbstractMappingOperationGenerator {
    public MappingOperationGenerator(final XsdsUtil config, final String basicFlowPackageName,
	    final ComplexType request, final ComplexType response, final ComplexContentList complexContentList,
	    final Log log) {
	super(config, basicFlowPackageName, request, response, complexContentList, log);
    }

    String generateImpl() {
	this.log.debug("+generateImpl");
	List<Entry<ComplexTypeChild, List<ComplexTypeChild>>> childrenTree = this.getChildrenTree();
	...
	for (ComplexTypeChild requestChild : GeneratorMapperMojo.getValidChildren(this.request)) {
	    if (!"mapping".equals(requestChild.getChildName())) {
		sb.append("\t\t/* Request contained value ").append(requestChild.getChildName()).append(". */\n");
		sb.append("\t\t");
	...
	if (mapping != null) {
	    ...
	    sb.append("\t\t");
	    if (importedClasses.contains(mapping.getComplexType().getClassNameFullQualified())) {
	    ...
	    for (ComplexTypeChild grandChild : GeneratorMapperMojo.getValidChildren(mapping.getComplexType())) {
		sb.append("\t\t/* Set ").append(grandChild.getChildName()).append(". */\n");
		sb.append(this.getComplexContentSetter("request_mapping", mapping.getComplexType(), mapping.isList(),
	...
	for (ComplexTypeChild responseChild : GeneratorMapperMojo.getValidChildren(this.response)) {
	    sb.append(this.getComplexContentSetter("response", this.response, false, responseChild, importedClasses));
	    String newParentName = new StringBuffer(32).append("response_").append(responseChild.getChildName())
		    .toString();
	    for (ComplexTypeChild grandChild : GeneratorMapperMojo.getValidChildren(responseChild.getComplexType())) {
		sb.append("\t\t/* Set ").append(grandChild.getChildName()).append(" into ")
			.append(responseChild.getChildName()).append(". */\n");
		sb.append(this.getComplexContentSetter(newParentName, responseChild.getComplexType(),
			responseChild.isList(), grandChild, importedClasses));
		sb.append("\n");
	    }
	}
	sb.append("\n");
	...
	File f = Util.getFile(outputDirectory, this.packageNameImpl,
		new StringBuffer().append(this.implName).append(".java").toString());
	this.log.info(new StringBuffer().append("Write Impl ").append(f.getAbsolutePath()));
	try {
    ...
    private String getComplexContentSetter(final String parentName, final ComplexType parent,
	    final boolean isParentList, final ComplexTypeChild child, final Set<String> importedClasses) {
	StringBuffer sb = new StringBuffer(1024);
	String childName = new StringBuffer(32).append(parentName).append("_").append(child.getChildName()).toString();
	ComplexContent cc = this.getMapperDefinition(child.getComplexType());
	sb.append("\t\t");
	if (child.isList()) {
	    sb.append("List<");
	}
	if (importedClasses.contains(child.getComplexType().getClassNameFullQualified())) {
	...
	if (cc == null || cc.isInterfaceType) {
		...
		if (s == null) {
			...
			sb.insert(0, "\t\t// TODO object creation entered here!\n");
		    }
		} else {
		    sb.append(s);
		    sb.append(";\n");
	    ...
	    if (i == 0) {
		sb.insert(0, "\t\t// TODO no matching object found in request to set!\n");
	    }
	} else {
	    sb.append("null;//");
	    sb.append(Util.lowerize(cc.interfaceName));
    ...
    private String getMatchingRequestParameter(final ComplexTypeChild destination) {
	String s = null;
	StringBuffer sb = new StringBuffer(256);
	for (ComplexTypeChild requestChild : GeneratorMapperMojo.getValidChildren(this.request)) {
	    if (destination.getComplexType().getClassNameFullQualified()
		    .equals(requestChild.getComplexType().getClassNameFullQualified())) {
		sb.append("request.");
		sb.append(requestChild.getGetterName());
		sb.append("()");
		s = sb.toString();
	...
	if (s == null) {
		...
		for (ComplexTypeChild grandChild : GeneratorMapperMojo
		    ...
		    if (destination.getComplexType().getClassNameFullQualified()
			    .equals(grandChild.getComplexType().getClassNameFullQualified())) {
	...
	sb.append(this.request.getPackageName().substring(0, this.request.getPackageName().length() - ".svc".length()));
	sb.append(".operation");
	return sb.toString();
    }
}
