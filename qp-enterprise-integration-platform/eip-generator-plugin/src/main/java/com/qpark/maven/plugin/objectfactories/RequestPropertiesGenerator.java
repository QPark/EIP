...
public class RequestPropertiesGenerator {
     ...
     * @param complexTypes
     */
    public RequestPropertiesGenerator(final XsdsUtil xsds, final File outputDirectory,
	    final String serviceIdCommonServices, final Log log) {
	this.xsds = xsds;
	...
	String failureType = new StringBuffer(64).append(this.serviceIdCommonServices.toLowerCase()).append(".")
		.append(this.xsds.getMessagePackageNameSuffix()).append(".FailureType").toString();
	ComplexType failureTypeCt = null;
	String responseMessage = new StringBuffer(64).append(this.serviceIdCommonServices.toLowerCase()).append(".")
		.append(this.xsds.getMessagePackageNameSuffix()).append(".ResponseMessage").toString();
	...
	for (ComplexType type : this.complexTypes) {
		...
		if (type.getClassNameFullQualified().endsWith(failureType)) {
		    failureTypeCt = type;
		} else if (type.getClassNameFullQualified().endsWith(responseMessage)) {
	...
	if (failureTypeCt != null && responseMessageCt != null) {
	    ...
	    sb.append("\t/**\n");
	    sb.append(
		    "\t * Add all the collected failures if not contained in the {@link ResponseMessage#getFailure()}\n");
	    ...
	    File f = Util.getFile(this.outputDirectory, this.xsds.getBasePackageName(), "RequestProperties.java");
