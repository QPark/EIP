...
public abstract class AbstractGenerator {
    ...
    AbstractGenerator(final XsdsUtil config, final ComplexContentList complexContentList, final Log log) {
	this.config = config;
	...
	ComplexContent cc = null;
	for (ComplexContent complexContent : this.complexContentList.getDirectMappings()) {
	    if (complexContent.ct.getClassNameFullQualified().equals(ct.getClassNameFullQualified())) {
		cc = complexContent;
		break;
	    }
	}
	if (cc == null) {
	    for (ComplexContent complexContent : this.complexContentList.getDefaultMappings()) {
		if (complexContent.ct.getClassNameFullQualified().equals(ct.getClassNameFullQualified())) {
		    cc = complexContent;
		    break;
		}
	...
	if (cc == null) {
	    for (ComplexContent complexContent : this.complexContentList.getComplexMappings()) {
		if (complexContent.ct.getClassNameFullQualified().equals(ct.getClassNameFullQualified())) {
		    cc = complexContent;
		    break;
		}
