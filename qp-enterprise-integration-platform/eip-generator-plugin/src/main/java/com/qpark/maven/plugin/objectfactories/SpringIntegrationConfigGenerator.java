...
public class SpringIntegrationConfigGenerator {
	...
	private String getMarshallerContextPath() {
		StringBuffer sb = new StringBuffer(1024);
		TreeSet<String> pkgs = new TreeSet<String>();
		for (ElementType element : this.config.getElementTypes()) {
			pkgs.add(element.getPackageName());
		}
		for (String pkg : pkgs) {
			if (sb.length() > 0) {
				sb.append(":");
	...
	public void generateService(final String serviceId) {
		...
		xml.append("\t<bean id=\"")
				.append(capitalizeName)
				.append("WebServiceSecurmentPropertyProvider\" class=\"com.qpark.eip.core.spring.security.DefaultEipSecurmentPropertyProvider\">\n");
		...
		xml.append("\t</bean>\n");
		xml.append("\t<bean id=\"eipCaller")
				.append(capitalizeName)
				.append("Wss4jSecurityInterceptor\" class=\"com.qpark.eip.core.spring.security.EipWss4jSecurityInterceptor\">\n");
		...
		router.append("\t<int:router id=\"internalEipCaller")
				.append(capitalizeName)
				.append("ResponseTypeRouter\" input-channel=\"internalEipCaller")
				.append(capitalizeName)
				.append("WsOutgoingChannel\" ref=\"internalEipCaller")
				.append(capitalizeName)
				...
				if (element.isRequest() && elementResponse != null) {
					...
					if (ctResponse != null && !ctResponse.isSimpleType()
						...
						channels.append("\t<int:channel id=\"internalEipCaller")
								...
								.append(Util.capitalize(element.getMethodName()))
								.append("ResponseChannel\" />\n");
						router.append("\t\t<int:mapping value=\"")
								.append(ctResponse.getClassNameFullQualified())
								.append("\" channel=\"internalEipCaller")
								.append(capitalizeName)
								.append(Util.capitalize(element.getMethodName()))
								.append("ResponseChannel\" />\n");
					}
		...
		sb.append(channels);
		sb.append(router);
		sb.append("\t<bean id=\"internalEipCaller")
				.append(capitalizeName)
