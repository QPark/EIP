...
public class WsServletXmlGenerator {
	...
	if (this.additionalWebservicePayloadInterceptors != null) {
	    String[] ss = this.additionalWebservicePayloadInterceptors.split("id=\\\"");
	    for (String s : ss) {
		if (s.indexOf('"') > 0) {
		    beanIds.add(s.substring(0, s.indexOf('"')).replaceAll("\\\n", "").replaceAll("\\\r", ""));
		}
	    }
	}
	return beanIds;
     ...
     * @param elementTypes
     */
    public WsServletXmlGenerator(final XsdsUtil config, final String basePackageName, final String serviceId,
	    final String serviceIdCommonServices, final boolean serviceCreationWithCommon,
	    final boolean disableWebservicePayloadValidation, final String webservicePayloadLoggerImplementation,
	    final String additionalWebservicePayloadInterceptors, final File outputDirectory,
	...
	if (webservicePayloadLoggerImplementation == null) {
	    ...
	    this.webservicePayloadLoggerImplementation = webservicePayloadLoggerImplementation.trim();
	}
	if (additionalWebservicePayloadInterceptors == null) {
	    this.additionalWebservicePayloadInterceptors = "";
	} else {
	    this.additionalWebservicePayloadInterceptors = additionalWebservicePayloadInterceptors.trim();
	}
    }
	...
	for (ElementType element : this.elementTypes) {
		...
		if (elementResponse != null) {
			...
			if (targetNamespace == null || !targetNamespace.equals(element.getTargetNamespace())) {
	...
	if (this.serviceId.length() > 0) {
		...
		sb.append("\t\t\t\t<value>/WEB-INF/classes");
		sb.append(Util.getRelativePathTranslated(this.config.getBaseDirectory(), f));
		sb.append("</value>\n");
	    }
	}
	sb.append("\t\t\t</list>\n");
