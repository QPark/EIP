 ...
 */
@Mojo(name = "xjc-eip", threadSafe = false, defaultPhase = LifecyclePhase.GENERATE_SOURCES, requiresDependencyResolution = ResolutionScope.COMPILE)
public class XjcMojo extends AbstractJavaGeneratorMojo {
     ...
     * <p>
     * Parameter holding a List of Filters, used to match all files under the
     * {@code xjbSources} directories which should <strong>not</strong> be
     * considered XJB files. (The filters identify files to exclude, and hence
     * this parameter is called {@code xjbExcludeFilters}). If a file matches at
     * least one of the supplied Filters, it is not considered an XJB file, and
     * therefore excluded from processing.
     * </p>
     * <p>
     * If not explicitly provided, the Mojo uses the value within
     * {@code STANDARD_XJB_EXCLUDE_FILTERS}.
     * </p>
     * <p>
     * <strong>Example:</strong> The following configuration would exclude any
     * XJB files whose names end with {@code xml} or {@code foo}:
     * </p>
     *
     * <pre>
     *     <code>
    ...
    private List<String> getSchemaList(final Map<String, XsdContainer> xsdContainerMap) {
	List<String> sources = new ArrayList<String>();
	StringBuffer sb = new StringBuffer(1024);
	Set<String> imported = new TreeSet<String>();
	if (!this.includeAllModels) {
	    for (XsdContainer xc : xsdContainerMap.values()) {
		if (XsdsUtil.isMessagePackageName(xc.getPackageName(), this.messagePackageNameSuffixes,
	...
	for (String string : imported) {
	    System.out.println("XSD: " + string);
	}
	return sources;
    }

    private void setImportedNamespaces(final XsdContainer container, final Map<String, XsdContainer> xsdContainerMap,
	    final Set<String> imported) {
	if (container != null) {
	    for (String namespace : container.getImportedTargetNamespaces()) {
		if (namespace != container.getTargetNamespace() && !imported.contains(namespace)) {
		    imported.add(namespace);
		    this.setImportedNamespaces(xsdContainerMap.get(namespace), xsdContainerMap, imported);
		}
	    }
	}
    }
	...
	final List<Filter<File>> excludePatterns = this.xjcSourceExcludeFilters == null
		? STANDARD_SOURCE_EXCLUDE_FILTERS : this.xjcSourceExcludeFilters;
	...
	return FileSystemUtilities.filterFiles(this.getProject().getBasedir(), this.getSchemaList(xsdContainerMap),
	...
	final List<Filter<File>> excludePatterns = this.xjbExcludeFilters == null ? STANDARD_XJB_EXCLUDE_FILTERS
		: this.xjbExcludeFilters;
	Filters.initialize(this.getLog(), excludePatterns);

	return FileSystemUtilities.filterFiles(this.getProject().getBasedir(), this.xjbSources, STANDARD_XJB_DIRECTORY,
