...
public class ModelObjectFactoryGenerator {
     ...
     * @param complexTypes
     */
    public ModelObjectFactoryGenerator(final XsdsUtil xsds, final File outputDirectory,
	...
	for (Entry<String, XsdContainer> xx : this.xsds.getXsdContainerMap().entrySet()) {
		...
		for (String imp : xx.getValue().getImportedTargetNamespaces()) {
		    importedModels.put(imp, xx.getValue().getFile().getAbsolutePath());
		}
	    }
	}
	...
	for (ComplexType type : this.complexTypes) {
		...
		mName = new StringBuffer(6 + cName.length()).append("create").append(cName).toString().replaceAll("\\.",
			"");
		mOfName = mName;
		...
		if (methodNames.contains(mName)) {
		    mName = new StringBuffer(6 + cName.length() + pName.length()).append("create")
			    .append(Util.capitalizePackageName(pName)).append(cName).toString().replaceAll("\\.", "");
		}
	...
	File f = Util.getFile(this.outputDirectory, this.xsds.getBasePackageName(), "ModelObjectFactory.java");
