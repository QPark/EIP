...
public class PropertyUserProvider implements EipUserProvider, ReInitalizeable {
	...
	static class UserDefinition {
		...
		/**
		 * Get the list of {@link GrantedAuthority} containing the roles of the user.
		 *
	...
	/**
	 * Get the user definition key out of the property name (<code>xxx</code> in the
	 * example).
	 *
	 * @param propertyName
	 *                         The property name.
	...
	private synchronized void createUsers() {
		...
		for (final Map.Entry<String, String> userDefinitionProperty : userDefinitionProperties.entrySet()) {
		...
		for (final Map.Entry<String, String> userDefinitionProperty : userDefinitionProperties.entrySet()) {
			if (userDefinitionProperty.getKey().endsWith("password")) {
				...
				if (parsedUserDefinitions.containsKey(userKey)) {
					...
					this.logger.error("No user setup for password property {}", userDefinitionProperty.getKey());
				}
			}
		}
		 ...
		 * parsedUserDefinitions any more, the user is removed out of the userMap.
		 */
		final List<String> validUserNames = parsedUserDefinitions.values().stream().map(UserDefinition::getName)
		...
		parsedUserDefinitions.values().stream().forEach(ud -> this.userDefinitionMap.put(ud.getName(), ud));
		this.logger.trace("-createUsers");
	}
	...
	private User getUser(final UserDefinition userDefinition) {
		return Optional.ofNullable(userDefinition).map(ud -> {
			...
			if (this.logger.isTraceEnabled()) {
				this.logger.trace("Created: {} {}", userDefinition.toString(), userDefinition.getRoles().stream()
