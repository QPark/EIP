...
public class FlowExecutionLog {
  ...
  private String getMethodName(final ProceedingJoinPoint joinPoint) {
    final String className = joinPoint.getSignature().getName();
    return className;
  }
  ...
  @Around(
      value = "execution(* com.qpark.eip.inf.Flow+.invokeFlow(..)) || execution(public * com.qpark.eip.inf.FlowGateway+.*(..))")
  public Object invokeFlowAspect(final ProceedingJoinPoint joinPoint) throws Throwable {
    final long start = System.currentTimeMillis();
    final String interfaceName = this.getInterfaceName(joinPoint);
    final String methodName = this.getMethodName(joinPoint);
    String userName = "";
