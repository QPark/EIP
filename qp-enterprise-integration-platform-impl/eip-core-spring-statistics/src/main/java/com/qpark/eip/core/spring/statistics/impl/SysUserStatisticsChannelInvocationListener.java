 ...
 * @author bhausen
 */
public class SysUserStatisticsChannelInvocationListener implements EipChannelInvocationListener {
   ...
   */
  public static void logSystemUserLogType(final SystemUserLogType log, final String resultString) {
    if (log != null) {
  ...
  private Logger logger = LoggerFactory.getLogger(SysUserStatisticsChannelInvocationListener.class);
   ...
   */
  @Override
  public void channelInvocation(final ChannelInvocation channel, final String userName,
      final int result) {
    final String channelName = EipRoleVoter.getChannelName(channel);
    final String serviceName = EipRoleVoter.getServiceName(channel, ".service.", ".msg.");
   ...
   *            see {@link EipRoleVoter#getResultString(int)}
   */
  public void submitSystemUserLogType(final SystemUserLogType log, final String resultString) {
    if (log != null) {
      new Thread(() -> {
        this.statisticsListener.addChannelInvocation(log);
      }).start();
      SysUserStatisticsChannelInvocationListener.logSystemUserLogType(log, resultString);
    }
  }
}
