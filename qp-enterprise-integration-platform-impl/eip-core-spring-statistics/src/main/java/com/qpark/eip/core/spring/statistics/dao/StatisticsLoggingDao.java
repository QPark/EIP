 ...
 * @author bhausen
 */
public class StatisticsLoggingDao extends AbstractEipDao implements StatisticsListener {
   ...
   */
  @Override
  public void addChannelInvocation(final ApplicationUserLogType log) {
   ...
   */
  @Override
  public void addChannelInvocation(final SystemUserLogType log) {
    ...
    predicates
        .add(cb.equal(c.<String>get(SystemUserLogType_.operationName), log.getOperationName()));
    predicates.add(cb.between(c.<Date>get(SystemUserLogType_.logDateItem),
        getDayStart(log.getLogDateItem()), getDayEnd(log.getLogDateItem())));
          ...
          for (int i = list.size() - 1; i >= 0; i--) {
            ...
            if (l != null && l.getHjid() != null) {
              if (persistence.getHjid().equals(l.getHjid())) {
                ...
                StatisticsLoggingDao.setupSystemUserLog(persistence, l);
                list.remove(i);
                this.em.remove(l);
              }
            }
  ...
  /**
   * @see com.qpark.eip.core.persistence.AbstractEipDao#doAsyncDatabaseOperation(java.lang.String, java.lang.Object)
   */
  @Override
  ...
  public void eraseApplicationUserLog(final Date toDate) {
    final CriteriaBuilder cb = this.em.getCriteriaBuilder();
    final CriteriaDelete<ApplicationUserLogType> q =
        cb.createCriteriaDelete(ApplicationUserLogType.class);
    final Root<ApplicationUserLogType> c = q.from(ApplicationUserLogType.class);
    q.where(cb.lessThan(c.<Date>get(ApplicationUserLogType_.stopItem), toDate), cb.equal(
        c.<String>get(ApplicationUserLogType_.context), this.contextNameProvider.getContextName()));
    try {
  ...
  /**
   * Erase all {@link SystemUserLogType}s of the application scope older
   * than the given date.
   *
   * @param toDate
   *            the date.
  ...
  public void eraseSystemUserLog(final Date toDate) {
    ...
    final Root<SystemUserLogType> c = q.from(SystemUserLogType.class);
    q.where(cb.lessThan(c.<Date>get(SystemUserLogType_.logDateItem), toDate), cb.equal(
        c.<String>get(SystemUserLogType_.context), this.contextNameProvider.getContextName()));
    try {
      this.em.createQuery(q).executeUpdate();
    } catch (final Exception e) {
