...
public class AppUserStatisticsChannelAdapter
	...
	static String getDuration(final long millis) {
		final String hmss = String.format("%03d:%02d:%02d.%03d",
	...
	/** The map of requests. */
	private final Map<UUID, SimpleEntry<String, Long>> requestMap = new ConcurrentHashMap<>();
	 ...
	 *
	 * @param log the {@link ApplicationUserLogType}.
	 */
	public static void logApplicationUserLogType(
	...
	public void postSend(final Message<?> message, final MessageChannel channel,
		...
		if (channel.toString().contains("Response") && requestId != null) {
			...
			if (entry != null) {
				...
				this.submitApplicationUserLogType(channelName, log);
			}
		}
	}
	 ...
	 * @param log the {@link ApplicationUserLogType}
	 */
	public void submitApplicationUserLogType(final String channelName,
			final ApplicationUserLogType log) {
		if (log != null) {
			new Thread(() -> {
