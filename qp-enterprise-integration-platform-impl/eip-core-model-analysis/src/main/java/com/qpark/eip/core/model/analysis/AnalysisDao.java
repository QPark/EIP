...
public class AnalysisDao implements ExtendedDataProviderModelAnalysis {
	...
	@PersistenceContext(unitName = EipModelAnalysisPersistenceConfig.PERSISTENCE_UNIT_NAME,
			name = EipModelAnalysisPersistenceConfig.ENTITY_MANAGER_FACTORY_NAME)
	private EntityManager em;

	/**
	 * Check if the {@link EnterpriseType} with given name and modelVersion exists.
	 *
	 * @param name
	 ...
	 */
	@Transactional(value = EipModelAnalysisPersistenceConfig.TRANSACTION_MANAGER_NAME,
	...
	public List<InterfaceMappingType> getFlowInterfaceMappingTypes(final String modelVersion, final String flowId) {
		...
		final List<String> flowMappingInterfaceMappingTypeIds = this.getFlowMappingInterfaceMappingTypeIds(modelVersion,
				flowProcessTypeIds);
		final CriteriaBuilder cb = this.em.getCriteriaBuilder();
	...
	public List<FlowType> getFlows(final Collection<String> flowNamePattern) {
		return this.getFlowByNamePattern(this.getLastModelVersion(), flowNamePattern);
	}
	...
	public List<InterfaceMappingType> getInterfaceMappings(final String flowId) {
		return this.getFlowInterfaceMappingTypes(this.getLastModelVersion(), flowId);
	}
	...
	public Optional<ServiceType> getService(final String serviceId) {
		final Optional<ServiceType> value = Optional
				.ofNullable(this.getServiceByServiceId(this.getLastModelVersion(), serviceId));
		return value;
	}
	...
	public List<String> getServiceIds() {
		...
		return services.stream().filter(s -> Objects.nonNull(s.getServiceId())).map(s -> s.getServiceId()).sorted()
	...
	public List<EnterpriseType> getEnterprises() {
		...
		final Root<EnterpriseType> f = q.from(EnterpriseType.class);
		q.orderBy(cb.asc(f.<String> get(EnterpriseType_.name)), cb.asc(f.<String> get(EnterpriseType_.modelVersion)));
