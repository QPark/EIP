...
public class EipRoleVoter extends RoleVoter {
	 ...
	 * @param object the object to get the channel name from.
	 ...
	 */
	public static String getOperationName(final String channelName,
		...
		if (j > 0) {
			for (int i = "eip".length() + serviceName.length(); i < l - j; i++) {
	...
	public static String getServiceName(final String channelName) {
		...
		if (channelName.startsWith("eip")) {
			service.append(Character.toLowerCase(channelName.charAt("eip"
					.length())));
			char ch;
			int l = channelName.length();
	...
	/**
	 * @param eipLimitedAccessDataProvider the eipLimitedAccessDataProvider to
	 *            set
	 */
	public void setEipLimitedAccessDataProvider(
			final EipLimitedAccessDataProvider eipLimitedAccessDataProvider) {
	...
	private void traceRoleSettings(final Authentication authentication,
			final String channelName,
			final Collection<ConfigAttribute> attributes) {
		this.logger.trace(" vote {} {}: required  [{}]", channelName,
				authentication.getName(), this.getRequiredRoles(attributes));
		this.logger.trace(" vote {} {}: userroles [{}]", channelName,
				authentication.getName(), this.getGrantedRoles(authentication));
	}
	...
	private void channelInvocation(final Object object, final String userName,
			final int result) {
		if (ChannelInvocation.class.isInstance(object)
				&& this.channelListeners != null) {
			for (EipChannelInvocationListener listener : this.channelListeners) {
				listener.channelInvocation(((ChannelInvocation) object),
						userName, result);
			}
		}
	}
	...
	public int vote(final Authentication authentication, final Object object,
		...
		this.logger.debug(" vote {} {}: Role based vote is {}", channelName,
				authentication.getName(), getResultString(result));
		if (this.eipLimitedAccessDataProvider != null
				&& (result == ACCESS_ABSTAIN || result == ACCESS_GRANTED)
			...
			int currentCalls = this.eipLimitedAccessDataProvider
					.getCurrentRequestNumber(userName, serviceName,
							operationName);
			int allowedCalls = this.eipLimitedAccessDataProvider
					.getAllowedRequestNumber(userName, serviceName,
							operationName);
			this.logger.trace(
					" vote {} {}: current calls={}, allowed calls={}",
					channelName, authentication.getName(), currentCalls,
					allowedCalls);
			...
			this.logger.debug(" vote {} {}: Limited access based vote is {}",
