...
public class HttpsRequester {
    ...
    private static String read(final String url, final String httpAuthBase64, final HostnameVerifier hostnameVerifier)
	    throws IOException {
	...
	if (httpAuthBase64 != null) {
	    connection.setRequestProperty("Authorization",
		    new StringBuffer(httpAuthBase64.length() + 6).append("Basic ").append(httpAuthBase64).toString());
	}
	...
	if (this.trustManager == null) {
		...
		this.httpAuthBase64 = new String(
			Base64.encode(new StringBuffer(256).append(this.httpAuthUser).append(":")
				.append(this.httpAuthPwd == null ? "" : this.httpAuthPwd).toString().getBytes("UTF-8")),
			"UTF-8");
	    }
