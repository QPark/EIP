...
public class EipX509TrustManager implements X509TrustManager, HostnameVerifier {
  ...
  private final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(EipX509TrustManager.class);
  ...
  public void init() throws Exception {
    ...
    /*
     * Iterate over the returned trust managers, look for an instance of X509TrustManager. If found,
  ...
  public void setKeystorePassword(final String keystorePassword) {
    if (keystorePassword != null) {
      this.keystorePassword =
          EipJasyptEncryptionProvider.decrypt(keystorePassword, this.properties).toCharArray();
    }
  }
  ...
  public boolean verify(final String hostname, final SSLSession session) {
    try {
      ...
      if (hostname != null && session != null && session.getPeerCertificateChain() != null
        ...
        final Certificate cert = this.ks.getCertificate(hostname);
        if (cert != null && cert.getPublicKey() != null) {
          final String ksPublicKey = cert.getPublicKey().toString();
          final String serverPublicKey =
              session.getPeerCertificateChain()[0].getPublicKey().toString();
        ...
        this.logger.debug(
            "verify no hostname({}) or session with PeerCertificateChain and PublicKey.", hostname);
      }
