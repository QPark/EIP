 ...
 * @author bhausen
 */
public class JAXBElementAwarePayloadTypeRouter
	...
	protected List<Object> getChannelKeys(final Message<?> message) {
		...
		return (closestMatch != null)
				? Collections.<Object> singletonList(closestMatch) : null;
	}

	private String findClosestMatch(final Class<?> type,
		...
		for (String candidate : this.getChannelMappings().keySet()) {
			if (isArray) {
				...
				candidate = candidate.substring(0,
						candidate.length() - ARRAY_SUFFIX.length());
