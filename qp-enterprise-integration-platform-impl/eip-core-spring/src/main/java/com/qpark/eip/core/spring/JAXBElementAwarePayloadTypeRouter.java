 ...
 * @author bhausen
 */
public class JAXBElementAwarePayloadTypeRouter extends
		AbstractMappingMessageRouter {
	private static final String ARRAY_SUFFIX = "[]";

	/**
	 * Selects the most appropriate channel name matching channel identifiers which are the fully
	 * qualified class names encountered while traversing the payload type hierarchy. To resolve
	 * ties and conflicts (e.g., Serializable and String) it will match: 1. Type name to channel
	 * identifier else... 2. Name of the subclass of the type to channel identifier else... 3. Name
	 * of the Interface of the type to channel identifier while also preferring direct interface
	 * over indirect subclass
	 */
	@Override
	protected List<Object> getChannelKeys(final Message<?> message) {
		...
		return closestMatch != null ? Collections
				.<Object> singletonList(closestMatch) : null;
	}
	...
	private String findClosestMatch(final Class<?> type, final boolean isArray) {
		...
		for (String candidate : this.getChannelMappings().keySet()) {
			...
			int typeDiffWeight = this.determineTypeDifferenceWeight(candidate,
					type, 0);
			...
			} else if (typeDiffWeight == minTypeDiffWeight
					&& typeDiffWeight != Integer.MAX_VALUE) {
		...
		if (matches.size() > 1) { // ambiguity
			throw new IllegalStateException(
					"Unresolvable ambiguity while attempting to find closest match for ["
	...
	private int determineTypeDifferenceWeight(final String candidate,
			final Class<?> type, final int level) {
		...
		for (Class<?> iface : type.getInterfaces()) {
			...
			for (Class<?> superInterface : iface.getInterfaces()) {
				int weight = this.determineTypeDifferenceWeight(candidate,
						superInterface, level + 3);
		...
		return this.determineTypeDifferenceWeight(candidate,
				type.getSuperclass(), level + 2);
	}
}
