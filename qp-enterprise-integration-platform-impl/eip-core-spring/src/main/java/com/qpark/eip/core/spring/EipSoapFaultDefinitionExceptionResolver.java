 ...
 * @author bhausen
 */
public class EipSoapFaultDefinitionExceptionResolver
    extends AbstractSoapFaultDefinitionExceptionResolver {
  /** The {@link Logger}. */
  private final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(EipSoapFaultDefinitionExceptionResolver.class);
  ...
  private SoapFaultDefinition getSoapFaultDefinition(final Throwable t) {
    ...
    if (XwsSecurityValidationException.class.isInstance(t)) {
      ...
      sfd =
          this.getSoapFaultDefinition(t, true, "E_SOAP_MESSAGE_VALIDATION_ERROR", (Object[]) null);
    } else {
      final FailureDescription fd =
          BaseFailureHandler.handleException(t, null, this.logger, (Object[]) null);
  ...
  private SoapFaultDefinition getSoapFaultDefinition(final Throwable t,
      final boolean addErrorDetails, final String code, final Object... data) {
    ...
    sfd.setFaultCode(SoapFaultDefinition.SERVER);
    if (addErrorDetails) {
      sfd.setFaultStringOrReason(new StringBuffer(256).append(fd.getUserMessage())
