 ...
 * @author bhausen
 */
public class EipSoapFaultDefinitionExceptionResolver extends
	...
	private SoapFaultDefinition getSoapFaultDefinition(final Throwable t) {
		...
		if (XwsSecurityValidationException.class.isInstance(t)) {
			sfd = this.getSoapFaultDefinition(t, false,
					"AUTHENTICATION_USERNAME_PASSWORD_TOKEN_FAILED",
					(Object[]) null);
		} else if (AccessDeniedException.class.isInstance(t)) {
			sfd = this.getSoapFaultDefinition(t, false,
					"AUTHORISATION_ACCESS_DENIED", (Object[]) null);
		} else if (UnmarshallingFailureException.class.isInstance(t)) {
			sfd = this.getSoapFaultDefinition(t, true,
					"E_SOAP_FAULT_MARSHALLING_ERROR", (Object[]) null);
		} else if (ValidationFailureException.class.isInstance(t)) {
			sfd = this.getSoapFaultDefinition(t, true,
					"E_SOAP_MESSAGE_VALIDATION_ERROR", (Object[]) null);
		} else {
			FailureDescription fd = BaseFailureHandler.handleException(t, null,
					this.logger, (Object[]) null);
	...
	private SoapFaultDefinition getSoapFaultDefinition(final Throwable t,
			final boolean addErrorDetails, final String code,
			final Object... data) {
		FailureDescription fd = BaseFailureHandler.getFailure(code, t, data);
		System.out.println(fd);
		SoapFaultDefinition sfd = new SoapFaultDefinition();
		sfd.setFaultCode(SoapFaultDefinition.SERVER);
		if (addErrorDetails) {
			sfd.setFaultStringOrReason(new StringBuffer(256)
					.append(fd.getUserMessage()).append("\nError details:\n")
	...
	protected SoapFaultDefinition getFaultDefinition(final Object endpoint,
		...
		if (MarshallingWebServiceInboundGateway.class.isInstance(endpoint)) {
			this.logger.error("MarshallingWebServiceInboundGateway: {}, {}",
					((MarshallingWebServiceInboundGateway) endpoint)
							.getComponentName(),
					((MarshallingWebServiceInboundGateway) endpoint)
							.getComponentType());
		} else {
		...
		SoapFaultDefinition sfd = this.getSoapFaultDefinition(ex);
		System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
		return sfd;
	}
}
