...
public class ByteUtil {
	...
	if (bitString != null && bitString.length() == 8 && bitString.matches("^[01]*$")) {
	    return (byte) Integer.parseInt(bitString, 2);
	} else {
	    throw new IllegalArgumentException("Argument needs to be a string of length 8 only containing 0 and 1.");
	}
    }
	...
	if (hexString != null && hexString.length() == 2 && hexString.matches("^[0-1A-Fa-f]*$")) {
