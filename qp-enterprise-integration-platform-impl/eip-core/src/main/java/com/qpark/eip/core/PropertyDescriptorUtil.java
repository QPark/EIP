...
public abstract class PropertyDescriptorUtil {
	...
	public static class ObjectProperties implements
	...
	public static TreeMap<String, ObjectProperties> getObjectProperties(
			final Object o) {
		if (o == null) {
	...
	public static Enum<?> getEnum(final Object o, final Class<?> cl)
			throws Exception {
		Enum<?> value = null;
		if (o != null) {
	...
	public static Object getObject(final Class<?> type)
	...
	public static Field getField(final Class<?> cl, final PropertyDescriptor pd) {
		...
		for (Field df : cl.getDeclaredFields()) {
			if (df.getName().equalsIgnoreCase(pd.getName())
					&& df.getType().equals(pd.getPropertyType())) {
				f = df;
				break;
			}
		}
		if (f == null
				&& cl.getSuperclass().getPackage().getName()
						.startsWith("com.ses.")) {
	...
	public static Class<?> getGenericType(final Class<?> cl,
		...
		if (f != null) {
			...
			if (type instanceof ParameterizedType) {
				for (Type t : ((ParameterizedType) type)
