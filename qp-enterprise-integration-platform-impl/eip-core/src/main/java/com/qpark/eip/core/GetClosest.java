...
public abstract class GetClosest {
	...
	System.out.println(getClosestIndex(BigDecimal.ONE, BigDecimal.TEN, null, BigDecimal.ZERO));
	System.out.println(getClosestIndex(BigDecimal.ONE, BigDecimal.ONE));
	System.out.println(getClosestIndex(BigDecimal.ONE, BigDecimal.ZERO, BigDecimal.ONE));
	System.out.println(getClosestIndex(BigDecimal.ONE, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ONE));
	System.out.println(
		getClosestIndex(BigDecimal.ONE, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ONE));
    }
     ...
     *         list.
     */
    public static int getClosestIndex(final BigDecimal ref, final BigDecimal... bigDecimals) {
     ...
     *         list.
     */
    public static int getClosestIndex(final BigDecimal ref, final Collection<BigDecimal> bigDecimals) {
	if (bigDecimals == null) {
	    return -1;
	} else if (bigDecimals.size() == 1) {
	    return 0;
	} else {
	    return getClosestIndex(ref, bigDecimals.toArray(new BigDecimal[bigDecimals.size()]));
	}
    }
     ...
     */
    public static int getClosestIndex(final double ref, final double... doubles) {
	if (doubles == null) {
	    return -1;
	    ...
	    return getClosestIndex(new BigDecimal(ref), bigDecimals.toArray(new BigDecimal[bigDecimals.size()]));
	}
    }
