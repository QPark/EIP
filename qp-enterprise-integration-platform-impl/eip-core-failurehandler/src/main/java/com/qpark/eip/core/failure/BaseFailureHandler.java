...
public abstract class BaseFailureHandler {
	...
	private static AbstractUnmarschaller MARSHALLER = new AbstractUnmarschaller() {
		...
		protected Class<?>[] getContextClasses() {
			return new Class<?>[] { FailureMessagePhraseableType.class,
					FailureMessagePhraseType.class,
					FailureMessageListType.class, FailureMessageSeverity.class,
	...
	private static FailureDescription handleSoapFaultException(
		...
		if (e.getCause() != null
			...
			fd = handleWebServiceIOException(
					...
					data);
		} else if (e.getCause() != null && WebServiceTransportException.class
				.isInstance(e.getCause())) {
			fd = getFailure("E_SOAP_FAULT_CREDENTIAL_ERROR", e, data);
		} else if (e.getCause() != null && UnmarshallingFailureException.class
				.isInstance(e.getCause())) {
	...
	private static FailureDescription handleWebServiceIOException(
			...
			final Logger log, final Object... data) {
		FailureDescription fd = null;
		if (e.getCause() != null && SSLHandshakeException.class
				.isInstance(e.getCause().getCause())) {
		...
		} else if (e.getCause() != null && e.getCause().getClass().getPackage()
				.getName().startsWith("java.net")) {
