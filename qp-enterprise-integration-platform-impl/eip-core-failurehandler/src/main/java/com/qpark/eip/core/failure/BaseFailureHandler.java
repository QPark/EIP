...
public abstract class BaseFailureHandler {
	...
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory
			.getLogger(BaseFailureHandler.class);
	...
	private static String checkUnnumberedBrackets(final String message) {
		...
		if (index > -1) {
			...
			index = s.indexOf(new StringBuffer().append("{").append(number)
					.append("}").toString());
			while (index > -1) {
				number++;
				index = s.indexOf(new StringBuffer().append("{").append(number)
			...
			index = s.indexOf("{}");
			while (index > -1) {
				s = s.replaceFirst("\\{\\}", new StringBuffer().append("{")
		...
		if (m != null) {
			if (m.getText() != null && m.getText().trim().length() > 0) {
				if (data != null && data.length > 0) {
					MessageFormat mf = new MessageFormat(
							checkUnnumberedBrackets(m.getText()));
			...
			if (m.getPhraseKey() != null) {
				for (String key : m.getPhraseKey()) {
					if (phrases.get(key) != null
							&& phrases.get(key).length() > 0) {
						if (sb.length() > 0) {
							sb.append(" ");
						}
					}
	...
	public static FailureDescription getFailure(final String code,
			final Object... data) {
	...
	public static FailureDescription getFailure(final String code,
		...
		if (code != null) {
			...
			if (fm == null) {
				...
				supportInfo.append("No definition found for code ")
						.append(code).append(" in failure-messages.xml.\n");
			}
		} else {
			fm = getFailureMessage(DEFAULT);
		}
		...
		if (fm.getSeverity() != null
				&& fm.getSeverity().equals(FailureMessageSeverity.WARNING)) {
		...
		if (fd.getSeverity().equals(FailureMessageSeverity.ERROR)) {
			if (t != null) {
				showFailureDescription(t.getClass().getSimpleName(),
						t.getMessage(), fd.getErrorCode(), fd.getSeverity()
								.name(), fd.getWhatHappened(),
						fd.getWhyHappened(), fd.getWhatToDo());
			} else {
				showFailureDescription("", "", fd.getErrorCode(), fd
						.getSeverity().name(), fd.getWhatHappened(),
						fd.getWhyHappened(), fd.getWhatToDo());
			}
		}
	...
	private static void addBaseMessages() {
		InputStream is = BaseFailureHandler.class
				.getResourceAsStream(new StringBuffer(32).append("/")
						.append(FAILURE_MESSAGES_XML_BASE).toString());
		if (is != null) {
			try {
				FailureMessageListType failuresMessageList = (FailureMessageListType) MARSHALLER
						.getValue(is, FailureMessageListType.class);
	...
	private static void addFailureMessageList(
			final FailureMessageListType failuresMessageList) {
		if (failuresMessageList != null) {
			for (FailureMessagePhraseType phr : failuresMessageList
					.getMessagePhrase()) {
	...
	public static void addFailureMessages(final InputStream is) {
		...
		if (is != null) {
			try {
				failuresMessageList = (FailureMessageListType) MARSHALLER
						.getValue(is, FailureMessageListType.class);
				addFailureMessageList(failuresMessageList);
			} catch (JAXBException e) {
				e.printStackTrace();
	...
	private static final FailureMessageListType getMessageListType(
			final String filename) throws JAXBException {
		InputStream is = BaseFailureHandler.class.getResourceAsStream(filename);

		FailureMessageListType edited = (FailureMessageListType) MARSHALLER
				.getValue(is, FailureMessageListType.class);
		return edited;
	}
	...
	public static FailureDescription handleException(final Throwable e,
		...
		if (InvocationTargetException.class.isInstance(e)) {
			...
			ArrayList<Object> list = new ArrayList<Object>(data == null ? 1
					: data.length + 1);
			if (data != null) {
			...
			fd = getFailure("E_SOAP_FAULT_CLIENT_ERROR", e,
			...
			fd = handleSoapFaultException((SoapFaultException) e, defaultCode,
					log, list.toArray(new Object[list.size()]));
			if (fd == null) {
	...
	private static FailureDescription handleSoapFaultException(
			final SoapFaultException e, final String defaultCode,
			final Logger log, final Object... data) {
		FailureDescription fd = null;
		if (e.getCause() != null
				&& WebServiceIOException.class.isInstance(e.getCause())) {
			fd = handleWebServiceIOException(
					(WebServiceIOException) e.getCause(), defaultCode, log,
					data);
		} else if (e.getCause() != null
				&& WebServiceTransportException.class.isInstance(e.getCause())) {
			fd = getFailure("E_SOAP_FAULT_CREDENTIAL_ERROR", e, data);
		} else if (e.getCause() != null
				&& UnmarshallingFailureException.class.isInstance(e.getCause())) {
	...
	private static FailureDescription handleWebServiceIOException(
			final WebServiceIOException e, final String defaultCode,
			final Logger log, final Object... data) {
		FailureDescription fd = null;
		if (e.getCause() != null
				&& SSLHandshakeException.class.isInstance(e.getCause()
						.getCause())) {
			fd = getFailure("E_SOAP_FAULT_HTTPS_CERTIFICATE_ERROR", e, data);
		} else if (e.getCause() != null
				&& ConnectException.class.isInstance(e.getCause().getCause())) {
